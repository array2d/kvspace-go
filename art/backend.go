...
func init() { kvspace.Register("art", Conn) }
...
func (b *backend) Get(prefix string, keys []string) []kvspace.XValue {
	b.mustBegin()
	defer b.end()
	...
	for i, key := range keys {
		full := b.resolvePathLocked(kvspace.JoinPath(prefix, key))
...
func (b *backend) List(prefix string, expandExt bool) []string {
	b.mustBegin()
	defer b.end()
	mustValidDirPath(prefix)
	b.store.mu.RLock()
